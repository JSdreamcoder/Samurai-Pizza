...
const usePizzaMutations = (): UsePizzaMutationsOutput => {
  ...
  const onCreatePizza = useCallback(
    (selectedPizza) => {
      try {
        if (!selectedPizza?.name) throw new Error('Invalid Name');
        if (!selectedPizza?.description) throw new Error('Invalid Description');
        if (!selectedPizza?.imgSrc) throw new Error('Invalid Image Link');
        if (!selectedPizza?.toppingIds) throw new Error('Invalid Topping');
        createPizza({
          variables: {
            createPizzaInput: {
