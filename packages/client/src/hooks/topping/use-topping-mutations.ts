...
interface UseToppingMutationsOutput {
  ...
  onUpdateTopping: (selectedTopping: any) => Promise<void>;
}
...
const useToppingMutations = (): UseToppingMutationsOutput => {
  ...
  const onCreateTopping = useCallback(
    (selectedTopping) => {
      try {
        if (selectedTopping?.name) throw new Error('Invalid Name');
        if (selectedTopping?.priceCents) throw new Error('Invalid Price');
        createTopping({
          variables: {
            createToppingInput: {
